    ...
    else
    {
        ...
        while( currNode != NULL )
        {
            ...
            else
            {
                ...
                mpiPi_abort( "new address node range [0x%016llx,0x%016llx] overlaps our range [0x%016llx,0x%016llx]\n",
                    lowAddr, highAddr,
...
AddrRangeMap_Find( struct AddrRangeMap* map, void* addr )
{
    ...
    while( currNode != NULL )
    {
        mpiPi_msg_debug( "AddrRangeMap::Find: comparing with [0x%016llx,0x%016llx]\n",
        currNode->lowAddr,
        currNode->highAddr );
...
{
    addrToSourceMap = (struct AddrRangeMap*)malloc(sizeof(struct AddrRangeMap));
                ...
                unsigned int lineNumber )
{
    ...
    AddrRangeMap_Add( addrToSourceMap,
                        addr,
                        addr,           /* we will patch range ends later */
                        newEntry );
}
                                ...
                                Dwarf_Addr maxAddress )
{
    ...
    if( node->leftChild != NULL )
    {
         ...
         */
        ret = AddrToSourceMap_PatchRangesAux( node->leftChild,
                                                node->lowAddr );
    }
    else
    {
     ...
     */
    mpiPi_msg_debug( "AddrRangeMap::PatchRanges: resetting range from [0x%016llx,0x%016llx] to [0x%016llx,0x%016llx]\n",
                        node->lowAddr, node->highAddr,
                        node->lowAddr, (maxAddress - 1) );
...
AddrToSourceMap_PatchRanges( void )
{
    ...
    if( addrToSourceMap->root != NULL )
    {
        ...
        AddrToSourceMap_PatchRangesAux( addrToSourceMap->root,
                                        maxAddress );
    }
}
...
FunctionMap_Add( const char* funcName,
                Dwarf_Addr lowAddress,
                Dwarf_Addr highAddress )
{
...
static
void
HandleFunctionDIE( Dwarf_Debug dwHandle,
                    Dwarf_Die currChildDIE )
{
    char* funcName = NULL;
    ...
    if( (dwDieNameRet == DW_DLV_OK) &&
        (dwDieLowAddrRet == DW_DLV_OK) &&
        (dwDieHighAddrRet == DW_DLV_OK) )
    {
...
open_dwarf_executable(char *fileName)
{
    ...
    while( 1 )
    {
        ...
        dwStatus = dwarf_next_cu_header( dwHandle,
                                            NULL,   /* cu_header_length */
                                            NULL,   /* version_stamp */
                                            NULL,   /* abbrev_offset */
                                            NULL,   /* address_size */
                                            &nextCompilationUnitHeaderOffset,
                                            NULL ); /* error object */
            ...
            for( i = 0; i < nLineEntries; i++ )
            {
                    ...
                    (lineSrcFileStatus == DW_DLV_OK) )
                {
                    ...
                    if( i < (nLineEntries - 1) )
                    {
                        ...
                        int nextLineAddrStatus = dwarf_lineaddr( lineEntries[i+1],
                                                &nextLineAddress,
                                                NULL );
         ...
         */
        dwStatus = dwarf_tag( currCompileUnitDIE,
                                &currDIETag,
                                NULL );
        assert( (dwStatus == DW_DLV_OK) && 
                (currDIETag == DW_TAG_compile_unit ) );

        /* access the first child DIE of the compile unit DIE */
        dwStatus = dwarf_child( currCompileUnitDIE,
                                &currChildDIE,
                                NULL );
        if( dwStatus == DW_DLV_NO_ENTRY )
        {
        ...
        while( dwStatus == DW_DLV_OK )
        {
            ...
            dwStatus = dwarf_tag( currChildDIE,
                                    &currDIETag,
                                    NULL );
            if( dwStatus == DW_DLV_OK )
            {
            ...
            {
                mpiPi_abort("unable to access next child DIE of current compilation unit\n" );
            }
